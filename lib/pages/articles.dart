...
class _ArticlesState extends State<Articles> {
  ...
  Future<Null> _getData() async {
    ...
    if (_sortBy == null) {
      if (_lastVisible == null)
        data = await firestore.collection(collectionName).orderBy(_orderBy, descending: _descending).limit(10).get();
      else
        data = await firestore.collection(collectionName).orderBy(_orderBy, descending: _descending).startAfter([_lastVisible![_orderBy]]).limit(10).get();
    } else {
      if (_lastVisible == null)
        data = await firestore.collection(collectionName).where('content type', isEqualTo: _sortBy).orderBy(_orderBy, descending: _descending).limit(10).get();
      else
        data = await firestore.collection(collectionName).where('content type', isEqualTo: _sortBy).orderBy(_orderBy, descending: _descending).startAfter([_lastVisible![_orderBy]]).limit(10).get();
    }
  ...
  handlePreview(Article d) async {
    await showArticlePreview(context, d.title, d.description, d.thumbnailImagelUrl, d.loves, d.sourceUrl ?? '', d.date, d.category, d.contentType, d.youtubeVideoUrl);
  }
    ...
    showDialog(
        ...
        builder: (BuildContext context) {
          return SimpleDialog(
            ...
            elevation: 0,
            children: <Widget>[
              Text('Add to Featured', style: TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.w900)),
              SizedBox(
                height: 10,
              ),
              Text('Do you Want to add this item to the featured list?', style: TextStyle(color: Colors.grey[900], fontSize: 16, fontWeight: FontWeight.w700)),
              SizedBox(
                height: 30,
              ),
              Center(
                    ...
                    onPressed: () async {
                      if (ab.userType == 'tester') {
                        ...
                        await context.read<AdminBloc>().getTotalDocuments('featured_count').then((itemCount) async {
                          if (itemCount <= 10) {
                            await context.read<AdminBloc>().addToFeaturedList(context, timestamp).then((value) => context.read<AdminBloc>().increaseCount('featured_count'));
                            Navigator.pop(context);
                ...
                children: <Widget>[
                  ...
                  Row(
                    children: <Widget>[
                      Container(
                        ...
                        child: Row(
                          ...
                          children: <Widget>[
                            ...
                            Text(
                              d.views == null ? "0" : d.views.toString(),
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: <Widget>[
                            ...
                            Text(
                              d.loves == null ? "0" : d.loves.toString(),
                              style: TextStyle(color: Colors.grey, fontSize: 13),
                            )
                          ],
                        ),
                      ...
                      InkWell(
                          child: Container(height: 35, width: 45, padding: EdgeInsets.only(left: 8, right: 8), decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(10)), child: Icon(Icons.remove_red_eye, size: 16, color: Colors.grey[800])),
                          onTap: () {
                            handlePreview(d);
                          }),
                      SizedBox(width: 10),
                      InkWell(
                        child: Container(height: 35, width: 45, padding: EdgeInsets.only(left: 8, right: 8), decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(10)), child: Icon(Icons.edit, size: 16, color: Colors.grey[800])),
                        onTap: () {
                      ...
                      SizedBox(width: 10),
                      InkWell(
                        child: Container(height: 35, width: 45, padding: EdgeInsets.only(left: 8, right: 8), decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(10)), child: Icon(Icons.delete, size: 16, color: Colors.grey[800])),
                        onTap: () {
                      ...
                      Container(
                        ...
                        child: TextButton.icon(onPressed: () => openFeaturedDialog(d.timestamp), icon: Icon(LineIcons.plus), label: Text('Add to Featured')),
                      ),
                    ],
                  ),
                ],
